...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    table = $('#zero-config').DataTable({
      ...
      ajax: {
        url: `<?= IP_BACKEND ?>Realisasi/GetAllRealisasi`,
        type: "POST",
        headers: {
  ...
  $('#addButton').click(() => {
    const PRK = function() {
      ...
      $.ajax({
        async: false,
        url: `<?= IP_BACKEND ?>Kontrak/GetAllKontrakNoDt/detail`,
        type: "POST",
        headers: {
          "Authorization": "Bearer <?= $this->session->userdata('accessToken') ?>",
        },
  ...
  $('#btn-simpan').click(function() {
    ...
    if (this.name == 'tambah') {
      $.ajax({
        url: `<?= IP_BACKEND ?>Realisasi/InsertRealisasi`,
        type: "POST",
        headers: {
          "Authorization": "Bearer <?= $this->session->userdata('accessToken') ?>",
        },
  ...
  $('select[name="dkCode"]').on("select2:select", function(e) {
    ...
    //     $.ajax({
    //       async: false,
    //       url: `<?= IP_BACKEND ?>Kontrak/GetOneKontrak/index/${id}`,
    //       type: "GET",
    //       headers: {
    //         "Authorization": "Bearer <?= $this->session->userdata('accessToken') ?>",
    //       },
